...
mn_x_tm1_shape = mn_x_tm1.shape
mn_x_in_tm1 = mn_x.reshape((mn_x_tm1_shape[0]*mn_x_tm1_shape[1], -1))
mn_x_1_in_tm1 = x_1.fprop([mn_x_in_tm1])
mn_x_2_in_tm1 = x_2.fprop([mn_x_1_in_tm1])
mn_x_3_in_tm1 = x_3.fprop([mn_x_2_in_tm1])
mn_x_4_in_tm1 = x_4.fprop([mn_x_3_in_tm1])
mn_x_4_in_tm1 = mn_x_4_in.reshape((mn_x_tm1_shape[0], mn_x_tm1_shape[1], -1))

mn_enc_0 = encoder.get_init_state(mn_batch_size)
mn_dec_0 = decoder.get_init_state(mn_batch_size)

((mn_enc_t, mn_dec_t, mn_phi_mu_t, mn_phi_sig_t, mn_kl_t, mn_theta_mu_t, mn_theta_sig_t), mn_updates) =\
    theano.scan(fn=inner_fn,
                sequences=[mn_x_4_in, mn_x_4_in_tm1],
                outputs_info=[mn_enc_0, mn_dec_0, None, None, None, None, None])

for k, v in mn_updates.iteritems():
    k.default_update = v

reshaped_mn_x = mn_x.reshape((mn_x.shape[0]*mn_x.shape[1], -1))
reshaped_mn_theta_mu = mn_theta_mu_t.reshape((mn_theta_mu_t.shape[0]*mn_theta_mu_t.shape[1], -1))
reshaped_mn_theta_sig = mn_theta_sig_t.reshape((mn_theta_sig_t.shape[0]*mn_theta_sig_t.shape[1], -1))

mn_recon = Gaussian(reshaped_mn_x, reshaped_mn_theta_mu, reshaped_mn_theta_sig)
...
max_theta_mu = mn_theta_mu_t.max()
mean_theta_mu = mn_theta_mu_t.mean()
min_theta_mu = mn_theta_mu_t.min()
max_theta_mu.name = 'max_theta_mu'
mean_theta_mu.name = 'mean_theta_mu'
min_theta_mu.name = 'min_theta_mu'

max_theta_sig = mn_theta_sig_t.max()
mean_theta_sig = mn_theta_sig_t.mean()
min_theta_sig = mn_theta_sig_t.min()
